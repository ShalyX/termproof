...
import type { EvidenceRecord, VerificationInput } from './types.ts';
...
type PersistedStateEnvelope = {
  ...
  acceptanceTerms: PersistedVerificationCase['acceptanceLedger']['terms'];
  acceptanceAudit: PersistedVerificationCase['acceptanceLedger']['audit'];
...
export class PostgresPersistenceAdapter implements PersistenceAdapter {
  ...
  constructor(config: string | PoolConfig) {
    this.pool = new Pool(typeof config === 'string'
      ? {
          ...
          max: Number(process.env.TERMPROOF_DB_POOL_MAX ?? DEFAULT_POOL_MAX),
    ...
    try {
      ...
      await client.query(
        ...
        [
          record.caseId,
          record.caseId,
          record.input.milestone,
          record.input.githubRepository,
          record.snapshot.verdict,
          record.snapshot.verdict,
          record.version,
          json(stateEnvelope(record)),
  ...
  async mutateCase(
    ...
    try {
      ...
      if (idempotencyKey) {
        ...
        if (prior.rowCount === 1) {
          ...
          if (row.state === 'COMPLETED' && row.response_json) {
            const response = row.response_json as { version?: number; snapshot?: VerificationCaseSnapshot };
      ...
      const update = await client.query(
        ...
        [
          updated.snapshot.verdict,
          updated.snapshot.verdict,
          updated.version,
          json(stateEnvelope(updated)),
          caseId,
          current.version,
        ],
      );
      ...
      await client.query(
        ...
        [
          ...
          json({
            ...
            fromVerdict: current.snapshot.verdict,
            toVerdict: updated.snapshot.verdict,
          }),
        ],
      );
  ...
  async consumeRateLimit(scopeKey: string, max: number, windowMs: number, now = new Date()): Promise<RateLimitDecision> {
    ...
    try {
      ...
      return {
        allowed: Boolean(result.rows[0].allowed),
        currentCount: Number(result.rows[0].current_count),
  ...
  private async appendVersionRows(
    client: PoolClient,
    record: PersistedVerificationCase,
    previous: PersistedVerificationCase | null,
  ): Promise<void> {
    ...
    // These are content-addressed facts of the submitted condition. The live
    // production schema enforces one immutable ledger/audit hash per case.
    if (!previous) {
      await client.query(
    ...
    for (const evidence of observations.values()) {
      ...
      await client.query(
        ...
        [
          ...
          version,
          observationId,
          evidence.requestFingerprint ?? null,
          observationId,
          evidence.source,
          evidence.revision,
          evidence.rawHash,
    ...
    for (const entry of newEvidence) {
      ...
      await client.query(
        ...
        [
          ...
          entry.claimId,
          entry.stepId,
          evidence.observationId ?? evidence.id,
          evidence.source,
          evidence.revision,
          evidence.result,
          evidence.rawHash,
          json(evidence.extractedFacts),
          json(evidence),
          stableJson(evidence),
          evidence.observedAt,
        ],
    ...
    await client.query(
      ...
      [
        ...
        version,
        snapshot.verdict,
        snapshot.provenance.policy.version,
        json({ verdict: snapshot.verdict, claims: snapshot.claims, coverage: snapshot.coverage ?? [] }),
      ],
    );
          ...
          await client.query(
            `update termproof.evidence_requests
                set version = $1,
                    ...
                    resolved_at = case when $2 = 'SATISFIED' then now() else resolved_at end,
                    updated_at = now()
...
function stateEnvelope(record: PersistedVerificationCase): PersistedStateEnvelope {
  return {
    version: record.version,
    input: record.input,
    acceptanceTerms: record.acceptanceLedger.terms,
    acceptanceAudit: record.acceptanceLedger.audit,
...
function hydrateCase(row: Record<string, unknown>): PersistedVerificationCase {
  ...
  if (!rawState || typeof rawState !== 'object' || Array.isArray(rawState)) {
    throw new PersistenceError('PERSISTENCE_UNAVAILABLE', 'Persisted verification case is malformed');
  }
  const state = rawState as Partial<PersistedStateEnvelope>;
  const snapshot = state.snapshot;
  if (!snapshot || typeof snapshot !== 'object' || !snapshot.plan) {
    throw new PersistenceError('PERSISTENCE_UNAVAILABLE', 'Persisted verification case is malformed');
  }

  const fallbackMilestone = String(row.submitted_condition ?? snapshot.milestone ?? '');
  const fallbackAnchor = String(row.anchor ?? '');
  const input = state.input && typeof state.input.milestone === 'string' && typeof state.input.githubRepository === 'string'
    ? state.input
    : { milestone: fallbackMilestone, githubRepository: fallbackAnchor };
  const rebuilt = buildAcceptanceTermLedger(input.milestone);
  const acceptanceLedger = {
    ...rebuilt,
    terms: Array.isArray(state.acceptanceTerms) ? state.acceptanceTerms : rebuilt.terms,
    audit: state.acceptanceAudit && typeof state.acceptanceAudit === 'object' ? state.acceptanceAudit : rebuilt.audit,
  };

  return {
    caseId: String(row.id),
    version: Number(state.version ?? row.version),
    input,
    plan: snapshot.plan,
    snapshot,
    acceptanceLedger,
